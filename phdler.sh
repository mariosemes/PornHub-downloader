...
if [ "$action" == 'start' ]
	then	
		...
		while IFS= read -r modellist; do
			youtube-dl $ytdlsetts --playlist-start 1 --playlist-end $lastvideos -o $dllocation'/'$modellist'/%(title)s.%(ext)s' 'https://www.pornhub.com/model/'$modellist'/videos/upload'
		done <$models_file

		echo Starting stars
		while IFS= read -r starslist; do
			youtube-dl $ytdlsetts --playlist-start 1 --playlist-end $lastvideos -o $dllocation'/'$starslist'/%(title)s.%(ext)s' 'https://www.pornhub.com/pornstar/'$starslist'/videos/upload'
		done <$stars_file

		echo Starting users
		while IFS= read -r userlist; do
			youtube-dl $ytdlsetts --playlist-start 1 --playlist-end $lastvideos -o $dllocation'/'$userlist'/%(title)s.%(ext)s' 'https://www.pornhub.com/users/'$userlist'/videos/public'
		done <$users_file

		echo Starting channels
		while IFS= read -r channellist; do
			youtube-dl $ytdlsetts --playlist-start 1 --playlist-end $lastvideos -o $dllocation'/'$channellist'/%(title)s.%(ext)s' 'https://www.pornhub.com/channels/'$channellist'/videos'
		done <$channels_file
